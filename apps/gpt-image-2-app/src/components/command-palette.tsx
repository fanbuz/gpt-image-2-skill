...
export function CommandPalette({
  ...
  const items = useMemo(() => {
    ...
    return rows.filter((item) => !query || item.label.toLowerCase().includes(query.toLowerCase()));
  ...
  const groups = useMemo(() => {
    return items.reduce<Record<string, { item: Item; index: number }[]>>((acc, item, index) => {
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className="flex items-center gap-2.5 px-4 py-3 border-b border-border-faint">
            <Icon name="search" size={16} aria-hidden="true" style={{ color: "var(--text-faint)" }} />
            <input
              id={inputId}
              value={query}
          ...
          >
            {items.length === 0 ? (
              <div className="py-6 text-center text-[12px] text-faint">没有匹配的命令。</div>
            ) : (
              Object.entries(groups).map(([groupName, entries]) => (
                <div key={groupName}>
                  <div className="t-caps px-2.5 py-1.5" aria-hidden="true">{groupName}</div>
                  {entries.map(({ item, index }) => {
                    ...
                    return (
                      ...
                      >
                        <Icon name={item.icon} size={14} aria-hidden="true" style={{ color: "var(--text-faint)" }} />
                        <span className="flex-1 truncate">{item.label}</span>
                        {isActive && <span className="kbd" aria-hidden="true">↵</span>}
                      </button>
                    );
                  })}
                </div>
